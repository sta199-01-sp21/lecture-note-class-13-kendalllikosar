...
What is the downside to using point estimates? Hard to pinpoint exact value.
...
to be left handed? Would you be surprised if only 3 of them were left handed? No, not a large difference.
Would you be surprised if 40 of them were left handed? Yes, far off from previous observation.
...
4. **Calculate the bounds of the XX% confidence interval** as the middle XX% 
   of the bootstrap distribution (XX is how confident you want ot be).
...
Did everyone get the same value? No, we did not. If fact, each time you sample (run the above
...
There is a 95% chance that the true median price of a 1 bedroom apartment is between \$2162 and \$2875.
...
- Sample size: 261 (mice not on treatment)
...
There is 95% confidence that the true proportion of all mice that dies while not on treatment is between 0.0690 and 0.0793.
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_live, level = .90)
ci_mice_lived
```
